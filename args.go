...
func parseArgs() parsedArgs {
	var query string
	hook := flag.Bool("hook", false, "Prints a hook that redirects 'shovel search' here")
	...
	if *help {
		...
		fmt.Println("    --known  \tOnly show results from known buckets            \t[default: false]")
...
const poshHook = `function shovel { if ($args[0] -eq "search") { shovel-search.exe @($args | Select-Object -Skip 1) } else { scoop.ps1 @args } }`
